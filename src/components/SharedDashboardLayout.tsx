...
type SharedDashboardLayoutProps = {
  children: React.ReactNode
  orgData: any
  onLogout: () => void
  onCreateWorkflow: () => void
  activeTab: 'overview' | 'workflows' | 'users'
}
...
const SharedDashboardLayout: React.FC<SharedDashboardLayoutProps> = ({ 
  ...
  const getButtonClass = (tab: string) => {
    return `w-full justify-start ${activeTab === tab ? 'bg-blue-50 text-blue-700' : ''}`
  }
      ...
      <div className="flex">
        ...
        <aside className="w-64 bg-white border-r min-h-screen">
          <nav className="p-4 space-y-2">
            ...
            <Button 
              variant="ghost" 
              className={getButtonClass('workflows')}
